source("C:/Users/Qba Liu/Documents/STUDIA/BIOINF_MASTER_UPWR/PRACA_MAGISTERSKA/mod.R")
...
tab <- data.frame(matrix(rep(0,length(train_sizes)*3), 
                         ...
                         ncol = 3))
colnames(tab) <- c("SampleSize", "mu", "var")
...
for(i in 1:length(train_sizes)){
  ...
  diffs <- 1:100
  for(j in 1:100){
    coefs_boot <- Bootstrap_Model(X_train, Y_train, n_iter = 10, alpha = alpha_param,
