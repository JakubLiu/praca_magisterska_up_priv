...
# this function returns a response which's noise is proportional to its value
def hetero(y, hetero_power):
    W = np.random.choice([-1,1], size = 1, replace = False)
...
param_of_interest = list(range(0.1,5.0,0.01))  # strength of heteroscedasticity coefficient
...
for i in range(0, len(param_of_interest)):
    ...
    cnt = cnt + 1
    hetero_pwr_i = param_of_interest[i]
    ...
    Y_train = hetero(Y_train, hetero_pwr_i)  # 'add' heteroscedasticity

    # create test data
    X_test = np.random.normal(loc=0, scale=1, size=(n,p))
    Y_test = X_test @ betas
    Y_test = Y_test + beta0
    Y_test = hetero(Y_test, hetero_pwr_i)  # 'add' heteroscedasticity
    ...
    tab[i,0] = hetero_pwr_i
    tab[i,1] = np.mean(diffs)
    tab[i,2] = np.var(diffs)

output_file_path = '/media/DANE/home/jliu/MASTER_THESIS/overfitting_remedy/SYSTEMATIC_SIM/OPTIMUM_SIM/VARIANCE/Variance_sim_mean.csv'
