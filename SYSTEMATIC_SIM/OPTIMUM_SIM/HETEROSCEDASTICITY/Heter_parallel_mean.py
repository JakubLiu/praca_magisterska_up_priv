...
def hetero(y, hetero_power):
    ...
    noise_mean = np.abs(y)*hetero_power*W
    noise = np.random.normal(loc=noise_mean, scale=1.0)
    y_new = y + noise
    return y_new


# parameters over which we iterate
param_of_interest = list(np.arange(start=0.1, stop=5.0, step = 0.01)) # strength of heteroscedasticity coefficient
