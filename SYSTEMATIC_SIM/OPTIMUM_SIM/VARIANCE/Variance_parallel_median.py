...
param_of_interest = list(range(1,101,1))  # variance
...
for i in range(0, len(param_of_interest)):
    ...
    cnt = cnt + 1
    variance_i = param_of_interest[i]
    
    # generate train dataset
    X_train = np.random.normal(loc=0, scale=variance_i, size=(n,p))
    ...
    # create test data
    X_test = np.random.normal(loc=0, scale=variance_i, size=(n,p))
    Y_test = X_test @ betas
    Y_test = Y_test + beta0
...
output_file_path = '/media/DANE/home/jliu/MASTER_THESIS/overfitting_remedy/SYSTEMATIC_SIM/OPTIMUM_SIM/VARIANCE/Variance_sim_median.csv'
