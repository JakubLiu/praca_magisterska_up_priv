...
param_of_interest = np.arange(0.0, 1.0, 0.01)  # predictor correlation
...
for i in range(0, len(param_of_interest)):
    ...
    cnt = cnt + 1
    pred_corr_i = param_of_interest[i]

    # create the covariance matrix
    mean_vec = np.zeros(p, dtype = np.int8)
    cov_matrix = np.zeros((p,p), dtype = np.float32)
    cov_matrix.fill(pred_corr_i)
    np.fill_diagonal(cov_matrix, variance)

    # generate data from multivariate normal with the given covariance matrix
    rng = np.random.default_rng()
    X_train = rng.multivariate_normal(mean = mean_vec,
                                        cov = cov_matrix,
                                        size = n)
    ...
    Y_train = Y_train + np.random.normal(loc = 0, scale = np.std(Y_train)*noise_perc, size=n)

    # create test data
    X_test = rng.multivariate_normal(mean = mean_vec,
                                        cov = cov_matrix,
                                        size = n)
    Y_test = X_test @ betas
    Y_test = Y_test + beta0
    Y_test = Y_test + np.random.normal(loc = 0, scale = np.std(Y_test)*noise_perc, size=n)
...
output_file_path = '/media/DANE/home/jliu/MASTER_THESIS/overfitting_remedy/SYSTEMATIC_SIM/OPTIMUM_SIM/PREDICTOR_CORRELATION/PredCor_sim_median.csv'
