...
param_of_interest = list(range(15,1000,5))  # the sample size n
...
for i in range(0, len(param_of_interest)):
    ...
    cnt = cnt + 1
    n_i = param_of_interest[i]
    
    # generate train dataset
    X_train = np.random.normal(loc=0, scale=1, size=(n_i,p))
    betas_true = np.random.uniform(low = -5.0, high = 5.0, size = p+1)
    beta0 = betas_true[0]
    betas = betas_true[1:]
    Y_train = X_train @ betas
    Y_train = Y_train + beta0
    Y_train = Y_train + np.random.normal(loc = 0, scale = np.std(Y_train)*0.1, size=n_i)

    # create test data
    X_test = np.random.normal(loc=0, scale=1, size=(n_i,p))
    Y_test = X_test @ betas
    Y_test = Y_test + beta0
    Y_test = Y_test + np.random.normal(loc = 0, scale = np.std(Y_test)*0.1, size=n_i)
    ...
    for j in range(0,n_opt_model_runs):
        opt = Opt.Optimum(X_train, Y_train, X_test, Y_test, alpha = 1.0,
                          location = location_param, n_iter = n_iter_extension,
...
output_file_path = '/media/DANE/home/jliu/MASTER_THESIS/overfitting_remedy/SYSTEMATIC_SIM/OPTIMUM_SIM/SAMPLE_SIZE/SampleSize_sim_median.csv'
