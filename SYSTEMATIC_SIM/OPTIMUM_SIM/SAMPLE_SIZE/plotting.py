...
df = pd.read_csv("C:/Users/Qba Liu/Documents/STUDIA/BIOINF_MASTER_UPWR/PRACA_MAGISTERSKA/SYSTEMATIC_SIM/OPTIMUM_SIM/SAMPLE_SIZE/SampleSize_sim_mean.csv")

plt.subplot(1,2,1)
plt.gca().set_facecolor('papayawhip')
plt.plot(df.iloc[:,0], df.iloc[:,1], color = 'blue')
plt.title("Mean of the empirical distribution of diff across the sample size.", fontsize = 10)
...
plt.title("Variance of the empirical distribution of diff across the sample size.", fontsize = 10)
plt.xlabel("Sample size")
plt.ylabel("Empirical variance")
plt.grid()

text1 = '\n'.join(["For each number of iterations (n_iter) of the model extension",
...
plt.gcf().text(0.18, 0.60, text1, transform=plt.gcf().transFigure, fontsize=8,
...
df2 = pd.read_csv("C:/Users/Qba Liu/Documents/STUDIA/BIOINF_MASTER_UPWR/PRACA_MAGISTERSKA/SYSTEMATIC_SIM/OPTIMUM_SIM/SAMPLE_SIZE/SampleSize_sim_median.csv")
...
plt.gcf().text(0.18, 0.50, text1, transform=plt.gcf().transFigure, fontsize=8,
              verticalalignment='top', bbox=dict(facecolor='w', edgecolor='black', boxstyle='round,pad=1'))
